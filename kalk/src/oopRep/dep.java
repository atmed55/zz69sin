...
import javax.swing.*; //подключение библиотеки JcheckBox - элемент, который может быть выбран или отменен, и который отображает его состояние для пользователя

import java.awt.*; //подключение библиотеки Color, использующейся для инкапсуляции цветов в SRGB цветового пространства по умолчанию или цвета в произвольных цветовых пространствах , идентифицируемых ColorSpace
import java.awt.event.*;

  //подключение библиотеки Font, представляющая шрифты, которые используются для визуализации текста видимым образом
...
//Инициализация содержимого фрейма.
	private void initialize() {
		...
		//Далее объявляется, создается и настраивается метка суммы вклада, содержащая текст: "Сумма вклада, руб"
		...
		//Далее создается и настраивается флажок для указания расчета суммы вклада на конец срока без ежемесячной капитализации
		...
		//Далее создается и настраивается метка суммы на конец срока, содержащая текст: "Сумма на конец срока"
